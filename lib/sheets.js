 ...
 * K Status | L TicketId | M EmailSent | N CheckedIn | O CheckedInAt
 */

const HEADER = [
  "Timestamp", "FullName", "Email", "MobileNumber", "WhatsappNumber",
  "Gender", "Age", "Governorate", "EducationalStage", "ConsentMediaUsage",
  "Status", "TicketId", "EmailSent", "CheckedIn", "CheckedInAt",
];
...
function strToBool(value) { return String(value).trim().toUpperCase() === "TRUE"; }
...
function rowToRegistration(row, rowNumber) {
  ...
  return {
    ...
    status: get(10) || "Pending",
    ticketId: get(11) || null,
    emailSent: strToBool(row[12]),
    checkedIn: strToBool(row[13]),
    checkedInAt: get(14) || null,
  };
}
...
async function appendRegistration(input) {
  ...
  const values = [
    timestamp, input.fullName, input.email, input.mobileNumber, input.whatsappNumber,
    input.gender, String(input.age), input.governorate, input.educationalStage,
    boolToStr(input.consentMediaUsage), "Pending", "", boolToStr(false), boolToStr(false), "",
  ];
...
async function updateRegistrationStatusColumns(id, fields) {
  ...
  const values = [
    fields.status, fields.ticketId ?? "", boolToStr(fields.emailSent),
    boolToStr(fields.checkedIn), fields.checkedInAt ?? "",
  ];

  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: `'${title}'!K${id}:O${id}`,
    valueInputOption: "RAW",
    requestBody: { values: [values] },
  });
}
